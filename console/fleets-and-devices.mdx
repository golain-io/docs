...
For Golain-managed certificate enrollment instead of manually copying MQTT passwords, configure Omega [JITR](/edge/jitr) with your org, project, and fleet IDs plus a unique `device_name`. JITR creates the device record for you — do not add the device here first, or enrollment fails with `DEVICE_NAME_ALREADY_IN_USE`.
