...
Devices with the **`device-rpc`** Omega module expose protobuf/gRPC services tunneled over MQTT. Cloud callers invoke **unary** methods via the **RPC host** `https://rpc.ilyama.golain.io` (sync), **server-streaming** methods via the `/streams/` SSE path, or enqueue unary calls for durable delivery when devices are offline — see [Queued invoke](/edge/device-rpc/queued-invoke). **Queued delivery is at-least-once; device handlers must implement idempotency.** See [Device RPC](/edge/device-rpc/overview). This is separate from legacy [shell RPC](/edge/remote-control) (`exec:<command>` over control topics).

## Assets

**Assets** model your physical hierarchy — plants, lines, machines, components — as a recursive tree that nests under a **Project**, the same scope as fleets and devices. An asset node can **link** to an existing Device, Fleet, or Project so you can track counters, maintenance, and alarms against the same entities you already manage. See [Assets & maintenance](/assets/overview).
...
|-------------|-------------|--------------|
| Golain cloud | `https://api.ilyama.golain.io` | `https://dev.zitadel.golain.io` (Zitadel) |
