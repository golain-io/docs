...
Devices with the **`device-grpc`** Omega module expose protobuf/gRPC services tunneled over MQTT. Cloud callers invoke unary methods via HTTP on port **8082**; the platform registers schemas automatically when the device announces. See [Device gRPC RPC](/edge/device-rpc/overview). This is separate from legacy [shell RPC](/edge/remote-control) (`exec:<command>` over control topics).
