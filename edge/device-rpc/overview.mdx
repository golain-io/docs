...
<CardGroup cols={2}>
  ...
  <Card title="Queued invoke" icon="clock" href="/edge/device-rpc/queued-invoke">
    Enqueue calls for offline devices — poll until `succeeded`. **At-least-once** delivery; devices must dedupe.
  </Card>
</CardGroup>
...
3. **Invoke a method** — `POST` to `:8082/projects/{project_id}/devices/{device_id}/{Service}/{Method}` → [Invoking methods](/edge/device-rpc/invoking-methods). For offline-tolerant delivery, use [Queued invoke](/edge/device-rpc/queued-invoke) instead.
...
- [Queued invoke](/edge/device-rpc/queued-invoke) — offline delivery, polling, idempotency
