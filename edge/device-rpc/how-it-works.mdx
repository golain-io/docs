...
When a caller uses the **enqueue** path on `:8082`, the platform persists the request in `rpc_queued_calls` and a background runner delivers it via the same `InvokeDeviceBridge` relay as sync invoke.
...
sequenceDiagram
  ...
  Client->>Proxy: POST .../enqueue/Service/Method
  Proxy->>Worker: EnqueueDeviceRPCCall
  Worker->>PG: INSERT rpc_queued_calls pending
  Proxy-->>Client: 202 call_id state expires_at

  loop Runner poll or device reconnect
    Worker->>PG: Claim due row in_flight
    Worker->>Broker: InvokeDeviceBridge
    Broker->>Dev: gRPC unary
    Dev-->>Broker: Response
    Worker->>PG: Mark succeeded or failed
  end

  Client->>Proxy: GET rpc/queued/call_id
  Proxy-->>Client: state grpc_status response
```

Queued RPC is **at-least-once**: stale `in_flight` rows are reclaimed after **120 s** and retried; the platform does not pass `call_id` to the device.

<Warning>
  Device handlers must be **idempotent** when called through the queued path. See [Queued invoke](/edge/device-rpc/queued-invoke).
</Warning>
