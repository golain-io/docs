...
1. Your app sends an HTTP POST with a JSON body to the **RPC host** (`https://rpc.ilyama.golain.io`) — `/{Service}/{Method}` in the URL, no `/core/api/v1` prefix. The proxy accepts Connect JSON (`application/json`), Connect binary (`application/proto`), or raw wire (`application/x-protobuf`) — see [Content-Type matrix](/edge/device-rpc/invoking-methods#content-type-matrix).
2. Golain checks your token, organization, and the matching device permission (`can_invoke_rpc` for invoke/enqueue/stream; `can_read_rpc` for management reads).
3. Golain delivers the call to the device and waits for the handler to finish.
4. Golain returns Connect JSON (default), Connect binary, or wire bytes depending on how you called.
...
1. Your app POSTs to the **RPC host** with `/streams/{Service}/{Method}` in the URL and `Accept: text/event-stream`. Send JSON (`application/json`, transcoded to wire) or naked protobuf wire (`application/proto` / `application/x-protobuf`).
...
|-------------|------------|-------|
| **Unary** | `POST https://rpc.ilyama.golain.io/.../{Service}/{Method}` | JSON, Connect binary, or raw wire — see [Content-Type matrix](/edge/device-rpc/api-reference#content-type-matrix) |
| **Server stream** | `POST https://rpc.ilyama.golain.io/.../streams/{Service}/{Method}` | SSE (`text/event-stream`); JSON or naked wire request |
