...
|-----------|------------|
| `GET …/rpc/services` | `can_read_rpc` on device |
| `GET …/rpc/sessions/{session_id}/invocations` | `can_read_rpc` on device |
| `GET …/rpc/queued` | `can_read_rpc` on device |
| `GET …/rpc/queued/{call_id}` | `can_read_rpc` on device |
...
`can_invoke_rpc` implies `can_read_rpc`, so callers who can invoke can also list services, poll queued calls, and read invocation history.

## Rate limits

| Surface | Approx. limit |
|---------|---------------|
| Management GETs (services, descriptors, invocations, queued) | 64 req/s |
| Reflect (`POST …/rpc/reflect`) | 12 req/s |
| Sync invoke (RPC host) | 32 req/s per org+device |
| Enqueue (RPC host) | 12 req/s per org+device |
| Server stream (RPC host) | 8 req/s per org+device |
...
| `application/proto` or `application/x-protobuf` | Naked wire passthrough (on this path `application/proto` is **not** Connect framing) | SSE, base64 protobuf `message` events |

Other stream Content-Types return **415** `unsupported content type`.
