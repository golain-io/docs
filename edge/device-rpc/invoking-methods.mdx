...
2. Your user has **`can_invoke_rpc`** on the target device (implies `can_read_rpc` for discovery/history).
...
On `/streams/`, `application/proto` means **naked wire** (unlike unary, where it means Connect binary framing). Other Content-Types return **415**.
...
| Enqueue method (RPC host) | `can_invoke_rpc` on device |
| List services | `can_read_rpc` on device |
| Poll / list queued calls | `can_read_rpc` on device |
| Query invocation history | `can_read_rpc` on device |
| Force schema reflect | `can_write_device_metadata` on device |

`can_invoke_rpc` implies `can_read_rpc`. Missing permission returns `403 forbidden`.
...
curl -sS \
  -H "Authorization: Bearer $TOKEN" \
  -H "ORG-ID: $ORG_ID" \
  "$API/core/api/v1/projects/$PROJECT_ID/devices/$DEVICE_ID/rpc/sessions/$SESSION_ID/invocations?page=1&limit=10"
```

**Optional filters** (AND together; omit = no constraint):
...
curl -sS \
  -H "Authorization: Bearer $TOKEN" \
  -H "ORG-ID: $ORG_ID" \
  "$API/core/api/v1/projects/$PROJECT_ID/devices/$DEVICE_ID/rpc/sessions/$SESSION_ID/invocations?service_name=golain.device.v1.EchoService&method_name=Echo&created_after=2026-07-01T00:00:00Z"
```

Response shape: `data.items[]` plus pagination `meta` (`current_page`, `items_per_page`, `total_items`, `total_pages`).

By default, audit is **metadata-only** — each row includes `service_name`, `method_name`, `grpc_status_code`, `latency_ms`, and `trace_id`. Request and response bodies are not stored or returned unless payload capture is enabled for the project.

### Enable payload capture (optional)

To store and return full bodies, enable the project feature (requires **`PROJECT_ADMIN`**):
...
curl -sS -X PUT \
  ...
  -H "Content-Type: application/json" \
  -d '{"value":"true"}' \
  "$API/core/api/v1/projects/$PROJECT_ID/features/feature.rpc.payload_capture.enabled"
```

Set `"value": "false"` to disable. Only registry keys with **`user_enabled: true`** are writable via the project features API — operator-controlled keys are not exposed here.

When enabled, bodies are captured at invoke time (capped at **64 KiB** each) and returned on the invocations query:

| Field | Shape |
|-------|-------|
| `req_payload` | JSON object for Connect JSON requests; base64 string for protobuf wire |
| `resp_payload` | JSON object for Connect JSON responses; base64 string for protobuf wire |

Omitted when capture was disabled at invoke time or the body was empty.
...
| 403 | `forbidden` | Missing `can_invoke_rpc` (invoke) or `can_read_rpc` (reads) |
