...
| `unsupported content type` (415) on stream | Unsupported `Content-Type` on `/streams/` (not JSON / `application/proto` / `application/x-protobuf`) | [Content-Type and transcoding](#content-type-and-transcoding) |
...
The RPC proxy transcodes request bodies by path and `Content-Type`. On **`/streams/`**, `application/proto` and `application/x-protobuf` both mean **naked protobuf wire** (passthrough). On **unary** invoke, `application/proto` means **Connect binary framing** (Vanguard) and `application/x-protobuf` is the naked-wire fast path. Other stream Content-Types (for example gRPC-Web) return **415**.

Unary sync invoke returns Connect JSON when you send `application/json` (or empty `Content-Type`). `application/x-protobuf` responds with `application/proto` wire bytes.
...
| Generated client, raw wire | either path | `application/x-protobuf` | naked protobuf request |
| Stream with naked `application/proto` | `/streams/{Service}/{Method}` | `application/proto` | naked protobuf request (not Connect-framed) |
...
Local dev: `export API=https://localhost:19090` and `export RPC_PROXY=http://localhost:8082`, then use `$API/core/api/v1/projects/...` for management routes.
