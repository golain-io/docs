...
| Queued call stuck `pending` / `failed` | Device offline past TTL | [Queued delivery](#queued-delivery) |
...
- `GET .../rpc/queued/{call_id}` stays `pending` or `failed` for a long time
- Call eventually `expired` — device was offline past `expires_at`
- Call `succeeded` but device performed the action twice

**Check:**

1. Device **online** on MQTT when runner attempts delivery (queued calls wake on reconnect).
2. **`expires_at`** not passed — default TTL is **900 s**; extend with `?ttl_seconds=` on enqueue.
3. **`attempts`** vs `max_attempts` (default **5**) — inspect `last_error` on poll response.
4. Row reclaimed from **`in_flight`** — after **120 s** stale reclaim, runner retries (duplicate device invoke).
...
# Poll status
curl -sS -H "Authorization: Bearer $TOKEN" -H "ORG-ID: $ORG_ID" \
  "$API/core/api/v1/projects/$PROJECT_ID/devices/$DEVICE_ID/rpc/queued/$CALL_ID" | jq .
```

Bring device online; runner wakes on MQTT connect. For long outages, enqueue with higher `ttl_seconds` (max **86400**).
...
- Motor started twice, counter incremented twice, or duplicate writes after a single enqueue
- `succeeded` state but operator reports duplicate real-world effects

**Cause:**

Queued RPC is **at-least-once**. The platform may invoke the device again after success if status persistence fails, or after stale `in_flight` reclaim.

**Fix (device firmware):**

1. Add an **idempotency key** field to request protos; cache results per key on the device.
2. Make handlers **naturally idempotent** where possible.
3. Do not rely on `call_id` — it is not forwarded to the device today.

→ [Queued invoke](/edge/device-rpc/queued-invoke) · [Omega setup — Idempotency](/edge/device-rpc/omega-setup#idempotency-for-queued-delivery)
...
- Queued delivery: [Queued invoke](/edge/device-rpc/queued-invoke)
