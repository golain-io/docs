...
| Staged rows growing, no written data | Not approved or materialization failing | [Materialization](#materialization) |
...
5. For telemetry lineages, query the lineage's `edge_ts_*` hypertable rather than the legacy shared telemetry table
...
3. Omega version implements `state_write` local apply
4. Verify the target row changed in local SQLite
5. On the next device upload, confirm the affected PK carries `lkcv`
...
If `snapshot_on_first_run` is disabled, triggers only capture writes after module install. Enable `snapshot_on_first_run: true` when you need Omega to seed existing rows once before incremental sync.
...
| Written view falls back to QueryScript | When `mirror-rows` is not the right surface for telemetry, TUI queries the lineage's `edge_ts_*` table by device |
