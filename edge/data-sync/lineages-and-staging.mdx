...
| Pipeline | MQTT topic | Capture on device | Cloud consumer | Typical destination |
|----------|------------|-------------------|----------------|-------------------|
| **A — Telemetry** | `sync/telemetry/batch` | Timestamp cursor, `batch_seq` | Edge worker telemetry ingest stream | Telemetry data points |
| **B — State rows** | `sync/rows/batch` | Trigger journal, `commit_seq`, `journal_epoch` | Edge worker state ingest queue | Mirror table or `edge_state_*` |

Schema governance, staging, replay, and backpressure are **lineage-scoped** — both pipelines share the same lineage row and review ticket for a given `(device, source_table)`.
...
| `paused` | New rows may be **staged** instead of written to mirror/telemetry; may or may not trigger device downlink |
...
| `edge_pending_controls` | Outstanding downlink pause/resume/url_grant/state_write |
