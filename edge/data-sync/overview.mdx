...
2. **Scaffold Omega** with `golain omega scaffold` or [install manually](/edge/install) — issues certs, downloads Omega + SQLite deps, and can seed a three-table verification DB with `--verify-tables`.
...
6. **Query data** — use `mirror-rows` for relational/state tables and QueryScript against the lineage's `edge_ts_*` hypertable for telemetry.
