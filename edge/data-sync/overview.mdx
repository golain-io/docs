...
flowchart LR
  ...
  subgraph cloud [Golain cloud]
    ...
    S[(Staged rows)]
    M[(Mirror / telemetry)]
    O -->|sync/* MQTT QoS 1| B --> W
    W --> S
    W --> M
  end
  subgraph ops [You]
    TUI[platform-tui Edge Sync]
    API[HTTP /edge/* API]
    TUI --> API --> W
  end
```
...
| **MQTT broker** | [golain-io/ilyama](https://github.com/golain-io/ilyama) | mTLS auth, topic ACL, ingress to worker queue |
...
2. **Provision mTLS certs** for the device (cert worker or JITR — see [provisioning checklist](/edge/data-sync/provisioning-checklist)).
