...
Paths in this reference are relative to `/core/api/v1`, except **Device RPC proxy** routes (unary invoke, SSE stream, enqueue) which use `https://rpc.ilyama.golain.io` with no `/core/api/v1` prefix. Endpoints are grouped by resource (Organizations, Projects, Devices, Device RPC, Edge replication, Integrations, OTA, and more) in the sidebar.
...
<CardGroup cols={2}>
  ...
  <Card title="Device RPC" icon="bolt" href="/edge/device-rpc/api-reference">
    Invoke, stream, enqueue, and management playground.
  </Card>
